/**
 * Sage Mapping Loader - Build dynamic sage → questions relationships
 *
 * SCOPE (v1.0):
 * - Read from: vaults/8sages/1-knowledge-base/index-sages/{sage}-questions.md
 * - Parse markdown to extract: questions linked to each sage
 * - Maintain mapping as sage-question relationships change
 *
 * NOTE:
 * This is SPECIFIC to sage-questions mapping, not a general vault loader.
 * To avoid confusion:
 * - sageMappingLoader → sage-questions relationships only
 * - Future: sessionsLoader, projectsLoader, etc. for other vault data
 * - Future: vaultIndex for full vault visualization
 *
 * By isolating scope, we can add new loaders without creating ambiguity.
 */

import mcpVaultClient from './mcpVaultClient';

class SageMappingLoader {
  constructor() {
    this.cache = new Map();
...
export default new SageMappingLoader();
